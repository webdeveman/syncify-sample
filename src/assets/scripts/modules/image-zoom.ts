export default function ImageZoom(container, opts) {
    ...
    function setup() {
        ...
        data.zoomLens.element.classList.add(...['bg-accent', 'opacity-30']);
    ...
    function kill() {
        ...
        if (data.zoomLens && data.zoomedImg) {
            (container.contains(data.zoomLens.element)) && container.removeChild(data.zoomLens.element);
    ...
    var events = {
        ...
        handleMouseLeave: function () {
            data.zoomedImg.element.style.display = 'none';
            data.zoomLens.element.style.display = 'none';
            if(options.deactivate) options.deactivate()
        },
